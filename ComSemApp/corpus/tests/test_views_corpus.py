...
class TestCredentials(BaseTestCase):
    ...
    corpus_url = reverse("administrator:corpus/search")
    ...
    def test_not_logged_in_fail(self):
        response = self.client.get(self.corpus_url)
        self.assertRedirects(response, '%s?next=%s' % (self.login_url, self.corpus_url))
